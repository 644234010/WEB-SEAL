        ...
        <tbody>
            @foreach ($productshoes as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('myproduct.edit', $item->id) }}" class="btn btn-warning btn-sm"
                            onclick="confirmEdit(event, '{{ route('myproduct.edit', $item->id) }}')">แก้ไขรายการสินค้า</a>
                    </td>
                    <td>
                        <form action="{{ route('myproduct.delete', $item->id) }}" method="POST" class="delete-form">
                            @csrf
