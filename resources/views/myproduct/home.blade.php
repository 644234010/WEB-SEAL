<x-app-layout>
    ...
    <div class="container ">
        ...
        <hr class="my-4">
        <form class="d-flex" role="search" method="POST" action="{{ route('myproduct.search') }}" onsubmit="return false;">
            @csrf
            ...
            <ul class="dropdown-menu">
                ...
                @foreach ($categories as $category)
                    <li><a class="dropdown-item filter-category" href="#"
                            data-category-id="{{ $category->id }}">{{ $category->categories_name }}</a></li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="table-responsive" id="products-list">
        @include('myproduct.products_list', ['productshoes' => $productshoes])
    </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function fetch_data(page, query = '', categoryId = 'all') {
                $.ajax({
                    url: "{{ route('myproduct.search') }}",
                    method: "POST",
                    data: {
                        page: page,
                        query: query,
            ...
            $('#search-input').on('keyup', function() {
                var query = $(this).val();
                fetch_data(1, query, currentCategoryId);
            });
            ...
            $(document).on('click', '.pagination a', function(event) {
                ...
                fetch_data(page, query, currentCategoryId);
            });

            $('.filter-category').on('click', function(e) {
                e.preventDefault();
                currentCategoryId = $(this).data('category-id');
                fetch_data(1, $('#search-input').val(), currentCategoryId);
            });
        ...
        function filterProducts(categoryId) {
            $.ajax({
                url: "{{ route('myproduct.filterByCategory', '') }}/" + categoryId,
                method: 'GET',
                success: function(data) {
                    $('#products-list').html(data);
                },
                error: function(xhr, status, error) {
                    ...
                    alert("เกิดข้อผิดพลาดในการดึงข้อมูลสินค้า กรุณาลองใหม่อีกครั้ง");
                }
            });
        }
