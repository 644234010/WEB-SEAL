    ...
    <div class="d-flex align-items-center justify-content-center min-vh-100">
        <div class="w-100" style="max-width: 600px;">
            <div class="card mt-4 p-4">
                ...
                <form method="POST" action="{{ route('myprofileuser.update') }}" id="update-form">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="address" class="form-label fw-semibold">ที่อยู่</label>
                        <input type="text" id="address" name="address" value="{{ $user->address }}" required
                            autocomplete="given-name" class="form-control" placeholder="ที่อยู่">
                    </div>
                    <div class="mb-3">
                        <label for="address1" class="form-label fw-semibold">ที่อยู่</label>
                        <input type="text" id="address1" name="address1" value="{{ $user->address1 }}" required
                            autocomplete="given-name" class="form-control" placeholder="ที่อยู่1">
                    </div>
                    <div class="mb-3">
                        <label for="address2" class="form-label fw-semibold">ที่อยู่</label>
                        <input type="text" id="address2" name="address2" value="{{ $user->address2 }}" required
                            autocomplete="given-name" class="form-control" placeholder="ที่อยู่2">
                    </div>
                    <div class="mb-3">
                        <label for="address3" class="form-label fw-semibold">ที่อยู่</label>
                        <input type="text" id="address3" name="address3" value="{{ $user->address3 }}" required
                            autocomplete="given-name" class="form-control" placeholder="ที่อยู่3">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="phone_number" class="form-label fw-semibold">เบอร์โทร</label>
                        <input type="text" id="phone_number" name="phone_number" value="{{ $user->phone_number }}"
                            required autocomplete="given-name" class="form-control" placeholder="เบอร์มือถือ" maxlength="10"
                    ...
                    <div class="mt-4 text-center">
                        ...
                        <a href="{{ route('myprofileuser.show') }}" class="btn btn-success"
...
<script>
    ...
    document.getElementById('back-button').addEventListener('click', function(event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '{{ route('myprofileuser.show') }}';
            }
        });
    });
</script>
